...
To better compare the efficacy of the run value in predicting a player's likelihood of winning MVP, we should compare the R squared value with the R squared value of plotting vote points with another statistic. A very popular statistic used in measuring the most valuable player is WAR (wins above replacements), a stat found on Baseball Reference (available in the tables we already wrangled). WAR is similar to run value, but instead calculates the wins a player should create for their team above the average player. A key difference in WAR and run value is that WAR just inputs observational stats, and does not include Statcast data. Also, WAR includes data on fielding and running, whereas the run value we use is only calculated when the batter is at the plate.

The following two graphs compare vote points with WAR, and similarly plot a line of best fit. Again, we will examine NL hitters separately from AL hitters.
